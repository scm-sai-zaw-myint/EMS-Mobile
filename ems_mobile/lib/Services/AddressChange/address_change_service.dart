...
class AddressChangeService extends GetxController {
  ...
  List<AddressChangeRoute> routeList = [];
  RxList<AddressChangeRoute> routes = RxList<AddressChangeRoute>([]);
  ...
  final _route = AddressChangeRoute.empty().obs;
  ...
  AddressChangeRoute get routeController => _route.value;
  ...
  Future<void> addressChangeRegister() async {
    ...
    routes.value = RxList<AddressChangeRoute>.from(
        (routeListData).map((x) => AddressChangeRoute.fromJson(x)));
  ...
  addRoute(AddressChangeRoute route) async {
    final prefs = await SharedPreferences.getInstance();
    // prefs.clear();
    routeList.add(route);
